...
from app.schemas.punto_venta import PuntoVentaCreate, PuntoVentaUpdate, PuntoVentaResponse
...
async def update_punto_venta(
    ...
    """
    result = await db.execute(
        select(PuntoVenta).where(PuntoVenta.id == punto_venta_id)
    )
