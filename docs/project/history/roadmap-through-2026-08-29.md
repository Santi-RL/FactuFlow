...
- [x] PF-01B cerrado: vocabularios canónicos, constraints de
      ...
      la ejecución registrada aprobó seguridad, backend, frontend y E2E; Clawpatch revalidó
...
- [x] **PF-19 — Elegibilidad RECE, rechazo preautorización y cierre legacy
  ...
  - [x] **PF-19B — elegibilidad RECE end-to-end:** los cortes B.1, B.2 y B.3
        ...
        `verificado_rece`, `no_rece` y `no_verificado`, conserva fuente y
        ...
        administrador puede acreditar producción con una constancia válida, no
        ...
        Responsable Inscripto, Exento en IVA y Monotributo, sin admitir
        coincidencias genéricas ni debilitar las guardas previas al CAE.
  ...
  - [ ] **PF-19D — autoridad WSFE y uso operativo de puntos de venta (P1
        fiscal-operativo, Nivel 2):** sustituir hacia adelante la regla de autoridad
        inicial de PF-19B sin reescribir su historia ni debilitar sus guardas. El
        manual oficial define `FEParamGetPtosVenta` como el recuperador autenticado
        de puntos asignados a Facturación Electrónica que soportan CAE o CAEA vía
        Web Services; por eso la respuesta completa, consistente y correspondiente
        al CUIT activo será la fuente de verdad para descubrir y validar puntos.
        La constancia PDF quedará como complemento descriptivo opcional.
    - **Autoridad y compatibilidad:** validar de forma explícita `EmisionTipo` y
      aceptar solo valores compatibles con el flujo CAE implementado por FactuFlow.
      Número, tipo de emisión, presencia, bloqueo y baja serán de solo lectura y
      podrán cambiar únicamente mediante ARCA. Una respuesta vacía, duplicada,
      inconsistente o fallida devolverá un error seguro y no modificará ningún
      punto. La habilitación local nunca podrá superar una señal técnica negativa.
    - **Comprobación inicial y frescura:** el primer snapshot exitoso de cada emisor
      exigirá que el usuario seleccione manualmente `Comprobar con ARCA`. Después se
      conserva el umbral técnico de 90 días, la preparación automática cuando esté
      desactualizado y el preflight final antes de cualquier camino que pueda llegar
      a `FECAESolicitar`. La falla previa a ARCA no creará operaciones, intentos,
      reservas ni solicitudes de CAE nuevas.
    - **Uso local compartido:** agregar una preferencia durable por emisor
      `Usar en FactuFlow`, independiente del estado ARCA y compartida por todos sus
      usuarios autorizados. Cualquier usuario autorizado podrá cambiarla mediante
      una acción explícita y confirmada dentro del editor; no se incorporan permisos
      por punto o por usuario. Los puntos CAE nuevos quedarán marcados para uso por
      defecto, aunque un bloqueo o baja temporal impida seleccionarlos. Una
      deshabilitación expresa persistirá frente a importaciones, comprobaciones y
      posteriores desbloqueos de ARCA.
    - **Contrato emitible:** los selectores exigirán simultáneamente compatibilidad
      CAE informada por ARCA, presencia activa, ausencia de bloqueo/baja, frescura
      técnica y `Usar en FactuFlow`. La constancia y los campos descriptivos no
      participarán en elegibilidad, numeración ni payload fiscal. Los puntos de
      otros sistemas se conservarán como información, pero nunca se ofrecerán para
      emitir.
    - **Constancia y edición descriptiva:** `Importar constancia` será opcional y
      completará domicilio, nombre de fantasía y puntos de otros sistemas sin
      cambiar la preferencia de uso ni el estado técnico. Los usuarios podrán
      completar manualmente los campos descriptivos; una constancia posterior los
      sobrescribirá y registrará su procedencia como `Informado por ARCA`. La UI
      mostrará de forma discreta `Informado por ARCA`, `Ingresado manualmente` o
      `Sin información`, sin exponer evidencia técnica en estados normales. Si se
      ofrece un nombre interno para mostrar, permanecerá separado del nombre de
      fantasía informado por ARCA.
    - **Listado y acciones:** mantener una sola tabla, con los puntos usados en
      FactuFlow como vista habitual y un filtro simple para mostrar todos. Conservar
      los estados breves y accionables de `0.3.2`; ubicar la acción de habilitar o
      deshabilitar dentro del editor y reservar el listado principal para consulta.
      El contador reflejará el filtro aplicado. Reordenamiento de columnas,
      microcopy, accesibilidad y otros ajustes visuales no bloqueantes continuarán
      dentro de los ítems generales de refinamiento UX y no demorarán la unidad
      fiscal-operativa.
    - **Borradores, perfiles y lotes:** deshabilitar un punto incrementará su
      revisión fiscal y bloqueará toda emisión nueva o continuación pre-ARCA sin
      sustituir silenciosamente el punto ni borrar trabajo. Formularios, perfiles y
      lotes conservarán sus datos en estado recuperable y explicarán que debe
      elegirse otro punto habilitado o volver a habilitarlo desde `Puntos de venta`.
      Si ARCA ya pudo haber recibido una solicitud, se conservará la incertidumbre,
      idempotencia y reconciliación vigentes; los replays terminales continuarán
      devolviendo su resultado durable.
    - **Migración y rollback:** aplicar una migración aditiva y conservadora. Los
      puntos Web Services existentes quedarán inicialmente marcados para uso,
      incluso si están temporalmente bloqueados o dados de baja; los de otros
      sistemas quedarán fuera. Conservar fechas de comprobación, historia y ledger
      PF-19B sin exigir otra constancia. No eliminar tablas, contratos ni datos
      legacy en el mismo corte. Ensayar upgrade y rollback en SQLite/PostgreSQL y
      validar producción por conteos y snapshots antes de cambiar la autoridad.
    - **Matriz mínima:** primera comprobación manual; CAE, CAEA, bloqueado, baja,
      ausencia, respuesta vacía/inconsistente; punto nuevo y existente; habilitado,
      deshabilitado y desbloqueo posterior; PDF opcional y precedencia sobre datos
      manuales; usuarios comunes y administradores; multiemisor; formularios,
      perfiles, individual, lotes, worker, reintentos, stale y carreras; cero CAE en
      abortos preautorización; migración/rollback; y QA de mensajes con datos
      sintéticos. Implementar después de PF-03B y antes de la unidad integrada
      PF-06/PF-07/PF-08.
  - **Matriz de cierre histórica PF-19A/PF-19B/PF-19C:** punto RECE/no RECE/no
    verificable; constancia y
...
- [x] Sincronización manual de puntos de venta ARCA validada desde UI y resuelta
      ...
      exige una constancia válida y no vence por tiempo. La comprobación técnica se
      recomienda cada 90 días y se ejecuta antes de emitir si está pendiente o
      desactualizada
...
- [x] **PF-16C — CI como barrera real:** la barrera ejecuta Ruff,
      ...
      Nivel 0. La ejecución registrada aprobó los seis jobs de la barrera básica.
...
- **Corte `v0.3.0`:** la CI Nivel 2 aprobó PostgreSQL/Runtime Smoke, el
...
- [x] Despliegue productivo separado de `v0.3.2` registrado en `vps-admin` el
      29/08/2026 sobre el SHA exacto del tag, sin cambios de base y con QA
      post-deploy sin emisiones ni solicitudes de CAE
...
- **`v0.3.0` publicado:** el alcance está congelado en
  ...
  `v0.3.0`.
- **Cierre `v0.3.0`:** el SHA funcional y `7f7b3808` cerraron la CI Nivel 2,
...
- **`v0.3.2` publicado:** comprobación previa a la selección y matriz
  ...
  dependencias ni configuración; el despliegue y su QA sin CAE quedaron
  registrados posteriormente en el plano de control `vps-admin`.
...
- [ ] **P3 opcional — numeración ARCA bajo demanda desde Puntos de venta:**
      reutilizar `FECompUltimoAutorizado` para mostrar, dentro del editor y solo
      ante una acción expresa, el último autorizado y el próximo número por tipo
      de comprobante relevante. No agregar columnas permanentes, consultas
      automáticas por cada punto/tipo ni convertirlo en requisito de habilitación.
      Distinguir esta ayuda operativa de la reconstrucción histórica P2, que puede
      consultar y persistir comprobantes emitidos fuera de FactuFlow. Es una idea
      posterior y no forma parte de las prioridades inmediatas.

## Prioridades inmediatas

1. Retomar PF-03 con PF-03B después de `v0.3.2`:
   ...
   continúa separado.
2. Implementar PF-19D como siguiente unidad fiscal-operativa después de PF-03B:
   adoptar `FEParamGetPtosVenta` como autoridad autenticada para puntos CAE,
   separar disponibilidad ARCA de `Usar en FactuFlow`, volver opcional la
   constancia y migrar producción de forma aditiva sin perder guardas ni historia.
   Los refinamientos visuales no bloqueantes y la consulta opcional de numeración
   no deben ampliar ni demorar este corte.
3. Conservar PF-16G aceptada el 10/08/2026; Node 24, auditorías, cobertura, CI
   ...
   `v0.3.0`.
4. Después de PF-19D, ejecutar primero la unidad integrada PF-06A/PF-08A/PF-06B/
