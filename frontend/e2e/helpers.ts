...
const getEmpresaActivaId = (route: Route) => {
  ...
  return Number(empresaHeader || adminUser.empresa_id || empresa.id);
};

export const mockApi = async (page: Page) => {
  const state = {
    empresas: [empresa, empresaSecundaria] as any[],
    clientes: [] as any[],
    nextClienteId: 1,
    puntosVenta: [
      {
  ...
  const handler = async (route: Route) => {
    ...
    if (path === "/api/auth/login" && method === "POST") {
      ...
      if (
        body.email === adminCredentials.email &&
    ...
    if (path === "/api/empresas" && method === "GET") {
      if (!hasAuthHeader(route)) return unauthorized(route);
      return jsonResponse(route, 200, state.empresas);
    }
    ...
    if (path === "/api/empresas/2" && method === "GET") {
      if (!hasAuthHeader(route)) return unauthorized(route);
      return jsonResponse(route, 200, empresaSecundaria);
    }
    ...
    if (
      path.startsWith("/api/certificados/verificar-conexion/") &&
      method === "POST"
    ) {
      if (!hasAuthHeader(route)) return unauthorized(route);
      return jsonResponse(route, 200, {
    ...
    if (path === "/api/lotes-comprobantes" && method === "GET") {
      if (!hasAuthHeader(route)) return unauthorized(route);
      const empresaId = getEmpresaActivaId(route);
      return jsonResponse(
        route,
        200,
        state.lotes
          .filter((lote) => lote.empresa_id === empresaId)
          .map(({ grupos, filas, ...lote }) => lote),
      );
    }

    if (path === "/api/lotes-comprobantes/plantilla" && method === "GET") {
      if (!hasAuthHeader(route)) return unauthorized(route);
      return route.fulfill({
        status: 200,
        contentType:
    ...
    if (path === "/api/lotes-comprobantes/validar" && method === "POST") {
      ...
      const empresaId = getEmpresaActivaId(route);
      const empresaActiva =
        state.empresas.find((item) => item.id === empresaId) || empresa;
      ...
      const grupo = {
        id: grupoId,
        comprobante_ref: `LOTE-${loteId.toString().padStart(3, "0")}`,
        orden: 1,
        estado: "validado",
        tipo_comprobante: 6,
        concepto: 1,
      ...
      const lote = {
        id: loteId,
        nombre_archivo: `lote-e2e-${loteId}.xlsx`,
        archivo_hash: `hash-${loteId}`,
        estado: "validado",
      ...
      return jsonResponse(route, 200, {
        lote: { ...lote, grupos: undefined, filas: undefined },
        puede_emitirse: true,
    ...
    if (
      ...
      lote.grupos_emitidos = lote.total_grupos;
      lote.mensaje_resumen = "Todos los comprobantes del lote fueron emitidos.";
      lote.started_at = now;
      lote.finished_at = now;
      lote.updated_at = now;
      lote.grupos = lote.grupos.map((grupo: any) => ({
        ...grupo,
        estado: "autorizado",
        cae: "12345678901234",
        numero_asignado: 1,
        comprobante_id: 99,
        mensajes_json: ["Comprobante autorizado", "CAE 12345678901234"],
      }));
      lote.filas = lote.filas.map((fila: any) => ({
        ...fila,
        estado: "autorizado",
        mensajes_json: ["Comprobante autorizado", "CAE 12345678901234"],
      }));
    ...
    if (
      /^\/api\/lotes-comprobantes\/\d+\/(resultados|archivo-observado)$/.test(
        path,
      ) &&
      method === "GET"
    ) {
      ...
      const loteId = Number(parts[3]);
      const lote = state.lotes.find((item) => item.id === loteId);
      if (!lote)
        return jsonResponse(route, 404, { detail: "Lote no encontrado" });

      if (parts[4] === "archivo-observado") {
        return route.fulfill({
          status: 200,
          contentType:
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          headers: corsHeaders,
          body: "archivo-observado",
        });
      }

      return jsonResponse(route, 200, lote);
    }

    if (/^\/api\/lotes-comprobantes\/\d+$/.test(path) && method === "GET") {
      if (!hasAuthHeader(route)) return unauthorized(route);
      const loteId = Number(path.split("/")[3]);
      const lote = state.lotes.find((item) => item.id === loteId);
      if (!lote)
