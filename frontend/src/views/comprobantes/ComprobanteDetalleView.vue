  ...
  <div>
    ...
    <div v-else-if="comprobante" class="space-y-6">
      ...
      <BaseCard>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            ...
            <p class="text-lg font-semibold text-gray-900">{{ tipoComprobanteNombre }}</p>
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-500 mb-1">Número</label>
            <p class="text-lg font-mono font-semibold text-gray-900">{{ numeroCompleto }}</p>
          </div>
      ...
      <BaseCard title="👤 Cliente">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <p class="text-gray-900">{{ comprobante.cliente_nombre }}</p>
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-500 mb-1">CUIT / Documento</label>
            <p class="text-gray-900">{{ comprobante.cliente_cuit }}</p>
          </div>
        </div>
      </BaseCard>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-4 py-3 text-right text-xs font-medium text-gray-700 uppercase">Precio Unit.</th>
                <th class="px-4 py-3 text-center text-xs font-medium text-gray-700 uppercase">IVA</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="item in comprobante.items" :key="item.id">
                <td class="px-4 py-3 text-sm text-gray-600">{{ item.codigo || '-' }}</td>
      ...
      <BaseCard title="💰 Totales">
        <div class="flex justify-end">
          <div class="w-80 space-y-3">
            ...
            <div v-if="comprobante.iva_21 > 0" class="flex justify-between text-gray-700">
              <span class="font-medium">IVA 21%:</span>
              <span class="font-mono">{{ formatMonto(comprobante.iva_21) }}</span>
            </div>

            <div v-if="comprobante.iva_10_5 > 0" class="flex justify-between text-gray-700">
              <span class="font-medium">IVA 10.5%:</span>
              <span class="font-mono">{{ formatMonto(comprobante.iva_10_5) }}</span>
            </div>

            <div v-if="comprobante.iva_27 > 0" class="flex justify-between text-gray-700">
      ...
      <BaseCard v-if="comprobante.observaciones" title="📝 Observaciones">
        <p class="text-gray-700 whitespace-pre-line">{{ comprobante.observaciones }}</p>
      </BaseCard>
    </div>
