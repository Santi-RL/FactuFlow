...
const formularioValido = computed(() => {
  return (
    cuitSinFormato.value.length === 11 && nombreEmpresa.value.trim().length > 0
  );
});
...
const puedeContinuar = computed(() => {
  return usarCSRExistente.value
    ? formularioManualValido.value
    : csrGenerado.value;
});
...
const generarCSR = async () => {
  ...
  try {
    ...
    error.value =
      ...
      "Error al generar CSR. Por favor, intentá nuevamente.";
  } finally {
    loading.value = false;
  }
};

const precargarEmisorActivo = () => {
  const emisor = emisorActivo.value;
  if (!emisor) return;

  if (!cuit.value) {
    cuit.value = formatearCUIT(emisor.cuit);
  }

  if (!nombreEmpresa.value) {
    nombreEmpresa.value = emisor.razon_social;
  }
};
...
const cargarClaves = async () => {
  ...
  try {
    ...
    errorClaves.value =
      err.response?.data?.detail || "No se pudieron listar las claves";
  } finally {
...
const descargarCSRNuevamente = () => {
  if (!csrData.value) return;
  certificadosService.descargarCSR(csrData.value.csr, cuitSinFormato.value);
};
...
<template>
  <div class="max-w-3xl mx-auto">
    ...
    <BaseAlert v-if="emisorActivo" type="info" class="mb-6">
      ...
      responsable o apoderado, salvo que esa persona sea quien va a emitir los
      comprobantes.
    </BaseAlert>
    ...
    >
      ...
      <form class="space-y-4" @submit.prevent="generarCSR">
        ...
        <BaseInput
          ...
          label="Nombre fiscal / razón social del emisor"
          placeholder="Nombre fiscal tal como figura en ARCA"
          required
          :maxlength="255"
        />
