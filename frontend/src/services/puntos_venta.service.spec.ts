...
describe("puntosVentaService", () => {
  ...
  it.each([false, true])(
    "envía siempre la confirmación de procedencia productiva explícita (%s)",
    async (confirmarProcedenciaProduccion) => {
      ...
      await expect(
        puntosVentaService.importarConstancia(file, {
          confirmar_procedencia_produccion: confirmarProcedenciaProduccion,
        }),
      ).resolves.toEqual(importResponseMock);
      ...
      expect(body.get("confirmar_procedencia_produccion")).toBe(
        String(confirmarProcedenciaProduccion),
      );
